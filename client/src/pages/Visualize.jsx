...
export default function Visualize() {
  ...
  const [narrativeGeneratedAt, setNarrativeGeneratedAt] = useState("");
  ...
  async function handleInterpretNarrative() {
    if (narrativeLoaded || narrativeLoading) return;
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        const detail = data.detail || data.error || "Could not interpret this PCA run.";
        setNarrativeError(detail);
        toast.error(detail, "Narrative unavailable");
        return;
      }
      ...
      toast.error("Could not reach the server.", "Narrative unavailable");
    } finally {
      setNarrativeLoading(false);
    }
  }
  ...
  return (
    <main className="app-shell workspace-shell">
      ...
      <section className="card viz-plot-card">
        <h3>Narrative</h3>
        <p className="muted">
          Generate a concise interpretation of the component structure, retention tradeoffs, and notable patterns in this run.
        </p>
        <button
          className="btn btn-primary"
          disabled={narrativeLoading}
          onClick={handleInterpretNarrative}
        >
          {narrativeLoading ? "Interpreting…" : "✦ Interpret Results"}
        </button>

        {(narrativeLoading || narrativeError || narrativeLoaded) && (
            ...
            {!narrativeLoading && !narrativeError && narrative && (
              <>
                ...
                <span className="narrative-meta">
                  Generated by Gemini · {narrativeGeneratedAt ? new Date(narrativeGeneratedAt).toLocaleString() : "Just now"}
                </span>
              </>
            )}
          </div>
